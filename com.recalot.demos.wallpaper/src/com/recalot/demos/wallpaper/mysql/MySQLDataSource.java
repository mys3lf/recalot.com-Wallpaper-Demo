...
package com.recalot.demos.wallpaper.mysql;
...
public class MySQLDataSource extends DataSource {
    ...
    private void readItems() {
        ...
        PreparedStatement statement = null;
        try {
            statement = connection.prepareStatement("SELECT * FROM " + sqlDatabase + ".items ORDER BY timeStamp DESC");
            result = statement.executeQuery();
            ...
            while (result.next()) {
                try {
                    ...
                    String id = result.getString(1);
                    String categories = result.getString(2);
                    String src = result.getString(3);
                    String title = result.getString(4);
                    String tags = result.getString(5);
                    String url = result.getString(6);
                    String timeStamp = "" + result.getTimestamp(7).getTime();
                    String content = result.getString(8);
...
//        return new Message("User successful saved", "User successful saved", Message.Status.INFO);
    ...
    private void putToSql(Item item) {
        ...
        try {
            ...
            statement.setString(1, item.getId());    //item id
            statement.setString(2, item.getValue("categories"));
            statement.setString(3, item.getValue("src"));
            statement.setString(4, item.getValue("title"));
            statement.setString(5, item.getValue("tags"));
            statement.setString(6, item.getValue("url"));

            String timestamp = item.getValue("timeStamp");

            if(timestamp != null && !timestamp.isEmpty()) {
                Long t = Long.parseLong(timestamp);
                if (t != null) {
                    statement.setTimestamp(7,  new Timestamp(new Date(t).getTime()));
                } else {
            ...
            statement.setString(8, item.getValue("content"));
    ...
    private void putToSql(Interaction interaction) {
        ...
        try {
            ...
            statement.setTimestamp(4, new Timestamp(interaction.getTimeStamp().getTime()));    //user_id
    ...
    private void updateAtSql(Item item) {
        ...
        try {
            ...
            statement.setString(1, item.getValue("categories"));
            statement.setString(2, item.getValue("src"));
            statement.setString(3, item.getValue("title"));
            statement.setString(4, item.getValue("tags"));
            statement.setString(5, item.getValue("url"));
            String timestamp = item.getValue("timeStamp");

            if(timestamp != null && !timestamp.isEmpty()) {
                Long t = Long.parseLong(timestamp);
                if (t != null) {
                    statement.setTimestamp(6,  new Timestamp(new Date(t).getTime()));
                } else {
            ...
            statement.setString(7, item.getValue("content"));
            statement.setString(8, item.getId());    //item id
